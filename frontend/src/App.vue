...
<template>
  <header class="app-header">
    <a class="brand" href="/">KOL 照妖镜</a>
    <nav>
      <a href="/">博主最近观点</a>
      <a href="/?view=queue">待处理队列</a>
      <a href="/?view=pinned">已钉住</a>
      <a href="/?view=raw">原始时间线</a>
      <a href="/?view=filtered">标签过滤流</a>
    </nav>
  ...
  <main>
    ...
    <template v-if="page?.view === 'authors'">
      <div class="page-title"><div><h1>博主最近观点</h1><p>选择博主，查看最近市场相关观点和后续变化。</p></div></div>
      <div class="author-layout">
        <aside class="panel">
          ...
          <a v-for="author in page.authors" :key="author.author_platform_uid" class="author-option" :class="{ active: author.author_platform_uid === page.selected?.author_platform_uid }" :href="`/?author=${encodeURIComponent(author.author_platform_uid)}`">
            ...
            <small>观点发言 {{ author.viewpoint_count }} · 已评估观点 {{ author.evaluated_viewpoint_count }}</small>
          </a>
        </aside>
        <section>
    ...
    <template v-else-if="page?.view === 'queue' || page?.view === 'pinned'">
      <div class="page-title"><div><h1>{{ page.view === "pinned" ? "已钉住" : "待处理注意力" }}</h1><p>围绕证据处置高信号版本。</p></div></div>
      <div class="toolbar">
      ...
      <div class="queue-layout">
        <section>
          ...
          <p v-if="!page.items.length">当前列表为空。</p>
        </section>
        <aside>
    ...
    <template v-else-if="page?.view === 'raw' || page?.view === 'filtered'">
      <div class="page-title"><div><h1>{{ page.view === "raw" ? "原始时间线" : "标签过滤流" }}</h1><p v-if="page.prompt_version">prompt 版本 {{ page.prompt_version }}</p></div></div>
      <TimelineCard v-for="item in page.items" :key="item.post_id" :item="item" :show-labels="page.view === 'filtered'" />
      <p v-if="!page.items.length">暂无记录。</p>
    </template>
    ...
    <template v-else-if="page?.view === 'post'">
      ...
      <section><h2>观察版本</h2><article v-for="version in page.card.versions" :key="version.version_id" class="card"><h3>观察版本 {{ version.version_id }}</h3><p class="muted">首次 {{ fmtTime(version.first_observed_at) }} · 最后 {{ fmtTime(version.last_observed_at) }}</p><pre>{{ version.content_text }}</pre><details><summary>相对上一版本 diff</summary><pre>{{ version.diff_from_prior_observed_version || "首个观察版本" }}</pre></details></article></section>
      <section v-for="name in ['feed_observations', 'direct_probes', 'events', 'attention_log', 'rewrite_exercises', 'enrichments']" :key="name"><h2>{{ name }}</h2><pre class="data">{{ JSON.stringify(page.card[name], null, 2) }}</pre></section>
    </template>
  </main>
</template>
