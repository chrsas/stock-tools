...
6. 健康门只限制负面推断。成功解析出的在场帖子始终保存 observation、版本与原始载荷；只有完整健康 run 才能生成缺席 observation、累计 streak、判定 `out_of_scope`。
...
运行配置在 `config/config.yml`、本地未提交配置和环境变量中维护。关键项包括追踪账号与轮询频率、监控窗口天数、缺席阈值 N、直链复查频率、TTL、样本门槛、LLM 供应商和模型、行情数据源、推送通道、watchlist、分析阈值、`llm.framework_prompt_version`、RSSHub 地址、Tier B 信源清单和 Telegram Bot Token。
