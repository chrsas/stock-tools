...
6. feed 轮询是增量的，翻到与上一轮已采集内容重合即停，不再横跨监控窗口，因此 feed 不做负面推断（不生成缺席 observation、不累计 streak、不由 feed 判定 `absent_confirmed` 或 `out_of_scope`）。成功解析或轻解析出的在场帖子参与本轮 seen set；首页指纹未变时每天最多完整观察首页一次；正向 observation 只在新帖、内容或图片变化、解析降级、状态恢复，或稳定旧帖达到观察间隔且未达到观察次数上限时追加。删帖与编辑的发现改由 Track B 按帖直链复查承担：每条帖在声称时间后 `first_recheck_after_days` 首检，之后每 `positive_observation_interval_days` 复查，满 `positive_observation_max_count` 次或确认移除后置 `inactive` 并 `out_of_scope`。
...
已完成阶段的详细规范和 DoD 在 `docs/phases-archive.md`。当前测试基线：pytest 398 通过，前端测试 8 通过。
...
运行配置在 `config/config.yml`、本地未提交配置和环境变量中维护。关键项包括追踪账号与博主级调度、请求预算、首轮种子窗口天数 `window_days`、复查生命周期参数 `first_recheck_after_days` / `positive_observation_interval_days` / `positive_observation_max_count`、每轮复查预算 `polling.max_probes_per_run`、TTL、样本门槛、LLM 供应商和模型、行情数据源、推送通道、watchlist、分析阈值、`llm.framework_prompt_version`、RSSHub 地址、Tier B 信源清单和 Telegram Bot Token。
...
2. 必测不变量：证据表不可改删；`posts` 禁删且身份字段不可改；A 到 B 到 A 版本链保留不同首次观察时间；preview 不建版本、不发 content 事件；feed 增量轮询翻到重合即停且不产生缺席 observation 或 streak；直链复查生命周期在声称时间后 `first_recheck_after_days` 首检、之后按 `positive_observation_interval_days` 复查、满 `positive_observation_max_count` 次或 `gone_confirmed` 后置 `inactive`+`out_of_scope`（置顶帖不自动退场）；`gone_confirmed` 只可由 reachable 翻回；同一帖至多一条 pending 队列项。
