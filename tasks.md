...
- **阶段 9**（统计分析层）：已完成。交付选择性删除分布、跨博主拥挤流水与单帖同标的历史面板。
- **阶段 10**（框架提取 enrich-v3）：已完成。交付 `framework.py`、`extract-frameworks` 命令、
  框架库页面（按主题/变量聚合、链回版本、来源状态标注）。
- 上述阶段全部 DoD 有自动化测试兜底（`tests/`，当前全套 256 通过）。
...
已完成。交付选择性删除分布、跨博主拥挤事件与单帖同标的历史面板；详细规范见
`docs/phases-archive.md`。

### 阶段 10：框架提取 enrich-v3

已完成。交付 `framework_extractions` + `framework_extraction_scans` 派生表（幂等键含
`prompt_version`，零结果扫描留痕不重试）、`extract-frameworks` CLI、框架库网页
（`?view=frameworks`，按主题/输入变量聚合，逐条链回 version_id，原帖不可读时框架仍可用并
以中性措辞标注来源状态）。prompt 版本走 `llm.framework_prompt_version`（默认 framework-v1），
升级后旧行保留、新版本重扫。
...
- 阶段 10：`llm.framework_prompt_version`（默认 framework-v1）。
