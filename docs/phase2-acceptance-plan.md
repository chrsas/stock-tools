...
规范来源：`tasks.md` 第 0 节、第 2.6 节、第 3 节阶段 2、阶段 2b 与第 6 节。阶段 2 的目标是让原始时间线、证据卡片、关注理由和单条改写训练形成可用闭环；阶段 2b 在此基础上补充轻量网页和手机私网访问。
...
- [x] 阅读 `tasks.md` 第 0 节、阶段 2、第 6 节。
...
- [ ] 连续运行多轮 `run-once`，确认新观察持续追加，时间线稳定可读。
...
- [ ] 新增 `serve --config-dir config` 启动命令，默认监听 `127.0.0.1`。
- [ ] 增加服务端渲染的原始时间线页面。
- [ ] 增加服务端渲染的证据卡片页面。
- [ ] 确认页面展示三维状态、诚实观察时间、删帖强弱信号、版本 diff、关联 run 和脱敏附注。
- [ ] 确认页面不输出 `raw_meta`、`raw_payload`、cookie、API Key 或本地配置内容。
- [ ] 为只读路由、脱敏和不存在的 `post_id` 增加自动化测试。
...
- [ ] 增加单条改写训练入口。
- [ ] 增加记录人工 verdict 入口。
- [ ] 所有修改状态的请求只接受 `POST`，并校验 CSRF token。
- [ ] 写操作继续复用阶段 2 服务层，不在网页路由中复制状态机逻辑。
- [ ] 为 CSRF、钉住、取消钉住、关注理由版本锁定、改写训练版本锁定和 verdict 增加自动化测试。

### F3. 手机窄屏检查

- [ ] 使用浏览器窄屏视口检查时间线、证据卡片和表单。
- [ ] 确认正文、状态标签和版本 diff 可读。
- [ ] 确认操作按钮不会误触，表单无需横向滚动。
...
- [ ] 运行质量门禁。
- [ ] 检查 Git 状态和差异。
- [ ] 确认凭据、本地配置、运行数据库、导出文件、快照和原始采集文件均未进入提交。
- [ ] 将网页和手机私网访问验收结果补充到“执行记录”。
